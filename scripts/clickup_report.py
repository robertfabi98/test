...
def build_client_message(folder_name: str, tasks: list) -> str:
    ...
    lines = [f"\U0001f4c1 *{folder_name}*", f"_{len(tasks)} task totali_"]

    if progress:
        lines += ["", "\U0001f7e2 *In lavorazione*"] + progress
    if review:
        lines += ["", "\U0001f535 *In revisione*"] + review
    if due_soon:
        lines += ["", "\U0001f534 *In scadenza (7 giorni)*"] + due_soon
    if todo:
        lines += ["", "⚪ *Da fare*"] + todo
...
def main():
    ...
    send_telegram(f"\U0001f4ca *Report Aloha Ltd*\n_{date_str}_")
    ...
    send_telegram(
        f"\U0001f4c8 *Riepilogo workspace*\n"
        f"Task totali: *{totals['tasks']}*\n"
        f"\U0001f7e2 In lavorazione: *{totals['progress']}*\n"
        f"\U0001f535 In revisione: *{totals['review']}*\n"
        f"\U0001f534 In scadenza: *{totals['due_soon']}*"
    )
