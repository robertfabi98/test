...
def build_client_message(folder_name: str, tasks: list) -> str:
    ...
    if review:
        lines += ["", "\U0001f535 *In revisione* _(richiede la tua attenzione)_"] + review
    if due_soon:
        lines += ["", "\U0001f534 *In scadenza (3 giorni)*"] + due_soon
...
def main():
    ...
    send_telegram(
        ...
        f"\U0001f534 In scadenza (3 gg): *{totals['due_soon']}*"
    )
